    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-4">
      <div className="flex flex-col md:flex-row justify-between gap-6">
        <div className="flex-grow grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div>
            <span className="text-[10px] font-bold uppercase tracking-wider text-gray-400 block mb-1">Customer</span>
            <p className="font-semibold text-gray-900">{item.customer_name}</p>
          </div>
          <div>
            <span className="text-[10px] font-bold uppercase tracking-wider text-gray-400 block mb-1">Product</span>
            <p className="text-gray-700">{item.product_name} <span className="text-gray-400 text-sm">(x{item.quantity})</span></p>
          </div>
          <div>
            <span className="text-[10px] font-bold uppercase tracking-wider text-gray-400 block mb-1">Total Amount</span>
            <PriceTag price={item.total_price} className="text-lg" />
          </div>
          <div className="md:col-span-2 lg:col-span-3">
            <span className="text-[10px] font-bold uppercase tracking-wider text-gray-400 block mb-1">Shipping Address</span>
        ...
        <div className="flex flex-col items-end gap-3 min-w-[150px]">
          <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${
            item.status === 'accept' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'
          }`}>
            {item.status}
          </span>
          <Button
